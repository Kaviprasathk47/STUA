...
const VehicleStats = ({ data }) => {
    if (!data || data.length === 0) {
        return (
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-full">
                <h3 className="text-lg font-semibold text-slate-800 mb-4">Vehicle Performance</h3>
                <div className="flex items-center justify-center h-40">
                    <p className="text-slate-500">No vehicle usage recorded.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-full">
            <h3 className="text-lg font-semibold text-slate-800 mb-6">Vehicle Performance</h3>
                            ...
                            <tr key={vehicle._id} className="hover:bg-slate-50 transition-colors">
                                <td className="py-4 pl-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-semibold text-slate-800">{vehicle.vehicleName}</p>
                                            <p className="text-xs text-slate-500">{vehicle.vehicleModel}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="py-4 text-slate-800 font-medium">{vehicle.trips}</td>
                                <td className="py-4 text-slate-800">{vehicle.distance.toFixed(1)} km</td>
                                <td className="py-4">
